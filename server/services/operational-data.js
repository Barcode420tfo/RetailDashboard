...
export async function loadOperationalData(account,region){
 ...
 return {...scoped,period:scopedPeriod(period,account,scoped),fullCalendar:period,leaders:leaders.filter(l=>region==='LAG'&&(!isRestricted(account)||l.scope===account.zone||l.scope===account.clusterName||scoped.clusters.some(c=>c.name===l.scope))),links:links.filter(l=>scoped.agents.some(a=>String(a._id)===String(l.agent)))};
}
...
export function buildOperations(data,{account,region,from='2026-09-01',asOf=data.period.asOf}){
 ...
 const staff=people.map(p=>{
  ...
  return {...p,...metrics(tx,target),daysSold:new Set(tx.map(t=>t.date)).size,average:tx.length?sum(tx)/tx.length:0,transactions:tx,clusterName:c.name||'Unmapped',zone:c.zone||'Unmapped',...inactivity({sales:p.transactions,workingDates:fullCalendar.workingDates,availableDates:fullCalendar.availableDates,asOf,joinedOn:p.joinedOn,status:p.status}),soldOnAsOf:p.transactions.some(t=>t.date===asOf),assignedStores:assigned.map(s=>({id:s.storeId,name:s.name,address:s.address||'Location not supplied',state:s.state})),reportedStores:reported,storeAssignmentStatus:assigned.length?'Linked':link?.status||'Assignment not confirmed'};
 });
 ...
 return {region,from,asOf,maxDate:period.asOf,scope:account.clusterName||account.zone||{LAG:'Lagos',NOR:'North',SSE:'South West / South East'}[region],regionalLead:region==='LAG'?'Olajide Tinuoye · CCO / Lagos RBM':null,summary:{...metrics(selected,target),totalMbes:staff.length,activeMbes:staff.filter(p=>p.status==='ACTIVE').length,inactiveMbes:staff.filter(p=>p.inactive).length,zeroSaleMbes:staff.filter(p=>!p.count).length,totalStores:stores.length,activeStores:stores.filter(s=>s.status==='ACTIVE').length,storeStatusUnknown:stores.filter(s=>s.status==='UNKNOWN').length,productiveStores:matchedStores.size,zeroSaleStores:stores.length-matchedStores.size,reportedStoreLabels:sourceStores.length},zones,clusters:groupRows,people:staff,stores:storeRows,transactions:tx,unassigned:{count:selected.length-staff.reduce((n,p)=>n+p.count,0),value:sum(selected)-staff.reduce((n,p)=>n+p.value,0)},notes:['Inactivity means at least 3 consecutive confirmed selling days without a sale, through the reporting date. Missing or partial uploads do not establish zero sales.','Store productivity uses unique master matches. No matched sale does not prove inactivity when store links are incomplete.','Cluster/MBE targets and store assignments are shown only where confirmed; missing values remain unavailable.']};
}
